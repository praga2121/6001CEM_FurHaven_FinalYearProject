...
Class Master extends DBConnection {
	...
	function place_order(){
		...
		if($save_order){
			...
			$data = '';
			$cart = $this->conn->query("SELECT c.*,p.title,i.price,p.id as pid from `cart` c inner join `inventory` i on 
			i.id=c.inventory_id inner join products p on p.id = i.product_id where c.client_id ='{$client_id}' ");
	...
	function update_order_status(){
		...
		$eta_date = date('Y-m-d H:i:s', strtotime($eta_date));
	
		$update = $this->conn->query("UPDATE `orders` SET `status` = '$status', `eta_date` = '$eta_date' WHERE id = '{$id}' ");
